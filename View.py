from tkinter import *
from math import floor
# ---------------------------- CONSTANTS ------------------------------- #
PINK = "#e2979c"
RED = "#e7305b"
GREEN = "#9bdeac"
YELLOW = "#f7f5dd"
FONT_NAME = "Courier"
WORK_MIN = 25
SHORT_BREAK_MIN = 5
LONG_BREAK_MIN = 20
reps = 0
timer = None

# ---------------------------- TIMER RESET ------------------------------- # 
def reset_timer():
    window.after_cancel(timer)

    tomato.itemconfig(tomato_text, text="00:00")

    global reps
    reps = 0

    check_marks.config(text="")

    timer_title.config(text="Timer")

# ---------------------------- TIMER MECHANISM ------------------------------- # 
def start_timer():
    global reps
    reps += 1

    if reps % 8 == 0:
        time = LONG_BREAK_MIN
        timer_title.config(text="Break", fg=RED)
    elif reps % 2 == 0:
        time = SHORT_BREAK_MIN
        timer_title.config(text="Break", fg=PINK)
    else:
        time = WORK_MIN
        timer_title.config(text="Work", fg=GREEN)



    seconde_time = 60 * time
    count_down(seconde_time)



# ---------------------------- COUNTDOWN MECHANISM ------------------------------- # 
def count_down(count):
    seconde = count % 60
    if count % 60 < 10:
        seconde = f'0{count%60}'

    timer_print = f"{floor(count/60)}:{seconde}"

    tomato.itemconfig(tomato_text, text=timer_print)
    if count > 0:
        global timer
        timer = window.after(1000, count_down, count-1)

    else:
        start_timer()
        work_sessions = floor(reps/2)
        marks = "".join('✔' for _ in range(work_sessions))

        check_marks.config(text=marks)

# ---------------------------- UI SETUP ------------------------------- #
window = Tk()
window.title("Pomodoro timer")
window.config(pady=50, padx=100, bg=YELLOW)

window.after(1000, )

timer_title = Label(text="Timer", fg=GREEN, bg=YELLOW, font=(FONT_NAME, 50))
timer_title.grid(column=1, row=0)


TOMATO_IMG = PhotoImage(file="tomato.png")
tomato = Canvas(width=200, height=224, bg=YELLOW, highlightthickness=0)
tomato.create_image(100, 112, image=TOMATO_IMG)
tomato_text = tomato.create_text(100, 130, text="00:00:00", fill="black", font=(FONT_NAME, 20, "normal"))
tomato.grid(column=1, row=1)


button_start = Button(text="Start", command=start_timer)
button_start.grid(column=0, row=2)

button_reset = Button(text="Reset", command=reset_timer)
button_reset.grid(column=2, row=2)

check_marks = Label(text="", bg=YELLOW, fg=GREEN)
check_marks.grid(column=1, row=3)

window.mainloop()